...
class TestHero(TestCase):
    def setUp(self):
        self.hero = Hero("Bat Icho", 30, 500, 5)
        self.enemy = Hero("nPoTo", 30, 500, 5)

    def test_init(self):
        self.assertEqual("Bat Icho", self.hero.username)
    ...
    def test_battle_enemy_health_less_than_one_exception_raise(self):
        ...
        with self.assertRaises(Exception) as e:
            self.hero.battle(self.enemy)
        self.assertEqual("You cannot fight nPoTo. He needs to rest", str(e.exception))
    ...
    def test_str(self):
        self.assertEqual(f"Hero Bat Icho: 30 lvl\nHealth: 500\nDamage: 5\n", str(self.hero))
